...
GO
-----------------------------------------------
select top 5* from PhieuDatDV p
join DatTiemPhong dk on p.MaPhieuDV = dk.MaPhieuDV
where p.TrangThai = N'Đang chờ'


select top 5* from DatKhamBenh order by NgayKham desc

exec sp_LayDanhSachVacxinTrongChiNhanh 'CN002'

select * from VacXin_ChiNhanh where MaCN = 'CN002'

exec sp_LayDashboardBacSi 'NV0094'

select * from TaiKhoan t join nguoidung n on t.mand = n.mand
...
select * from fn_LayDanhSachKhamTrongNgay('NV0094')
SELECT 
        p.MaPhieuDV,
        CONVERT(VARCHAR(5), ISNULL(k.NgayKham, t.NgayTiem), 108) AS GioDat, 
		ISNULL(k.NgayKham, t.NgayTiem),
    ...
    LEFT JOIN DatKhamBenh k ON p.MaPhieuDV = k.MaPhieuDV 
    LEFT JOIN DatTiemPhong t ON p.MaPhieuDV = t.MaPhieuDV 
    WHERE p.BacSiPhuTrach = 'NV0094' and p.TrangThai != N'Đã thanh toán' and p.TrangThai != N'Hoàn thành'
	order by ISNULL(k.NgayKham, t.NgayTiem) desc

--test sp_sp_CapNhatChuanDoanKham
select * from PhieuDatDV where MaPhieuDV = 'PDV003978'
select * from DatKhamBenh where MaPhieuDV = 'PDV052527'
--test sp_ThemLichTiemVaccine
exec sp_ThemLichTiemVaccine 'DK00200', 'VX02'

select * from ThoiGianTiemChiDinh where MaDK = 'DK00200'
delete ThoiGianTiemChiDinh where MaDK = 'DK00200'
select * from VacXin_ChiNhanh where MaCN = 'CN002'

select * from GoiTiemPhong
select top 5* from DangKyGoiTP order by madk desc
select * from PhieuDatDV where TrangThai


select top 5 * from DatTiemPhong where MaDK is null

exec sp_LayThongTinGoiTiemThuCung 'TC04354'


select top 5 * from DatTiemPhong where MaDK = 'DK00132' and MaTC = 'TC04354'
select top 5 * from DanhSachVacXin;




select top 5 * from ThoiGianTiemChiDinh where MaDK = 'DK00132' 
...
exec sp_LayDanhSachThuocTrongChiNhanh 'CN003'

select * 
FROM SanPham sp
INNER JOIN LoaiSP lsp ON sp.MaLoaiSP = lsp.MaLoaiSP
INNER JOIN SPCuaTungCN sctc ON sp.MaSP = sctc.MaSP
where sctc.MaCN = 'CN001' and lsp.TenLoaiSP = N'Thuốc'

select * from PhieuDatDV where TrangThai = N'Đang thực hiện'

select * from DonThuoc where MaPhieuDV = 'PDV003978' and MaCN = 'CN002'


select top 5* from DatKhamBenh where MaPhieuDV = 'PDV052527'
select * from  PhieuDatDV pd left join DonThuoc dt on pd.MaPhieuDV = dt.MaPhieuDV
where pd.TrangThai = N'Đang thực hiện' and pd.LoaiDichVu = N'Khám bệnh' and pd.MaPhieuDV = 'PDV052527',
