...
const AppointmentList = () => {
  ...
  return (
    <StaffLayout>
      {/* Header Section - Đã thu nhỏ padding/margin */}
      ...
      {/* Filter Section - Đã lược bỏ List/Grid và fix Date Picker */}
      ...
      {/* Stats Summary - Kích thước nhỏ gọn hơn */}
      ...
      {/* Appointment List - Thu nhỏ padding và text size */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        <div className="p-5 border-b border-gray-50 bg-gray-50/10">
          <h3 className="text-base font-bold text-gray-800">Danh sách lịch hẹn hôm nay (3)</h3>
        </div>
        
        <div className="divide-y divide-gray-50">
          {appointments.map((appt) => (
            <div key={appt.id} className="p-5 flex items-center justify-between hover:bg-blue-50/10 transition-all group">
              <div className="flex items-center gap-8">
                <div className="text-center w-16">
                  <p className="text-xl font-bold text-[#0095FF]">{appt.time}</p>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{appt.day}</p>
                </div>

                <div className={`w-1 h-12 rounded-full ${appt.color === 'blue' ? 'bg-blue-500' : appt.color === 'emerald' ? 'bg-emerald-500' : 'bg-orange-400'}`} />
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="bg-gray-100 px-2 py-0.5 rounded-lg text-[9px] font-bold text-gray-500 uppercase">{appt.service}</span>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-4 mt-1">
                      ...
                      {appt.doctor && <div className="text-xs text-blue-500 font-semibold">{appt.doctor}</div>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                <span className={`px-4 py-1.5 rounded-xl text-[10px] font-bold uppercase ${
                  appt.status === 'Đang thực hiện' ? 'bg-orange-50 text-orange-500' : 'bg-blue-50 text-blue-600'
                }`}>
                  {appt.status}
                </span>
                <div className="relative">
                  ...
                  {openMenuId === appt.id && (
                    <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-100 shadow-xl rounded-xl z-50 py-1">
                      <button onClick={() => handleAction('reschedule', appt)} className="w-full flex items-center gap-3 px-4 py-2.5 text-xs font-bold text-gray-600 hover:bg-gray-50 hover:text-[#0095FF]">
      ...
      {/* MODAL ĐỔI LỊCH HẸN - Đã tinh chỉnh kích thước gọn gàng */}
      {showReschedule && (
          ...
          <div className="bg-white rounded-[24px] w-full max-w-[380px] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-50 flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-gray-400 font-bold mt-1 uppercase tracking-wider">{selectedAppt?.pet} - {selectedAppt?.owner}</p>
              </div>
              <button onClick={() => setShowReschedule(false)} className="text-gray-400 hover:text-gray-800 p-1 bg-gray-50 rounded-full transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                <p className="text-[9px] text-gray-400 font-bold uppercase mb-1">Lịch hẹn hiện tại</p>
                <p className="font-bold text-gray-700 text-sm">Thứ Ba, 31/12/2024 lúc {selectedAppt?.time}</p>
              </div>

              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block ml-1">Ngày mới</label>
                  <input type="date" className="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-700 focus:ring-2 focus:ring-blue-500/10 outline-none" />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5 block ml-1">Giờ mới</label>
                  <input type="time" className="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-700 focus:ring-2 focus:ring-blue-500/10 outline-none" />
                </div>
              </div>
            </div>
