...
IF DB_ID (N'PetCareX') IS NOT NULL BEGIN
...
CREATE TABLE ChiNhanh (
    ...
    CONSTRAINT CK_ChiNhanh_SDT_So CHECK (
        SDT IS NULL
        OR SDT NOT LIKE '%[^0-9]%'
    ),
    CONSTRAINT CK_ChiNhanh_Gio CHECK (
        ThoiGianMo IS NULL
        OR ThoiGianDong IS NULL
...
CREATE TABLE LoaiThanhVien (
    ...
    CONSTRAINT CK_LoaiTV_Ten CHECK (
        TenLoaiTV IN (
            N'Cơ bản',
            N'Thân thiết',
            N'VIP'
...
CREATE TABLE KhuyenMai (
    ...
    CONSTRAINT CK_KhuyenMai_PhanTram CHECK (
        PhanTramGiam BETWEEN 0 AND 100
    )
);
GO
...
CREATE TABLE LoaiSP (
    ...
    CONSTRAINT CK_LoaiSP_Ten CHECK (
        TenLoaiSP IN (
            N'Thuốc',
            N'Thức ăn',
            N'Phụ kiện'
...
CREATE TABLE NguoiDung (
    ...
    CONSTRAINT CK_NguoiDung_LoaiND CHECK (
        LoaiND IN (N'Khách hàng', N'Nhân viên')
    ),
    CONSTRAINT CK_NguoiDung_NgaySinh CHECK (
        NgaySinh < CAST(GETDATE () AS date)
    ),
    CONSTRAINT CK_NguoiDung_SDT_So CHECK (
        SDT IS NULL
        OR SDT NOT LIKE '%[^0-9]%'
    ),
    CONSTRAINT CK_NguoiDung_CCCD_So CHECK (
        CCCD IS NULL
        OR CCCD NOT LIKE '%[^0-9]%'
    )
);
GO
...
CREATE TABLE TaiKhoan (
    ...
    CONSTRAINT CK_TaiKhoan_TrangThai CHECK (
        TrangThai IN (N'Hoạt động', N'Khóa')
    )
);
GO
...
CREATE TABLE KhachHang (
    ...
    CONSTRAINT CK_KhachHang_LanCuoi CHECK (
        LanCuoiToiCuaHang IS NULL
        OR LanCuoiToiCuaHang <= GETDATE ()
    ),
...
CREATE TABLE DichVu (
    ...
    CONSTRAINT CK_DichVu_Loai CHECK (
        TenDV IN (
            N'Khám bệnh',
            N'Tiêm Phòng',
            N'Mua hàng'
...
CREATE TABLE PhieuDatDV (
    ...
    CONSTRAINT CK_PhieuDatDV_HinhThuc CHECK (
        HinhThucDat IN (N'Tại quầy', N'Online')
    ),
    CONSTRAINT CK_PhieuDatDV_TrangThai CHECK (
        TrangThai IN (
            N'Đang chờ',
            N'Xác nhận',
            N'Hủy',
    ...
    CONSTRAINT CK_PhieuDatDV_Loai CHECK (
        LoaiDichVu IN (
            N'Khám bệnh',
            N'Tiêm Phòng',
            N'Mua hàng'
...
CREATE TABLE PhieuDatDVKhamBenh (
    MaPhieuDV varchar(10) NOT NULL,
    ...
    CONSTRAINT CK_PhieuKham_NTK CHECK (
        NgayTaiKham IS NULL
        OR NgayTaiKham > NgayKham
    ),
...
CREATE TABLE PhieuDatDVMuaHang (
    MaPhieuDV varchar(10) NOT NULL,
...
CREATE TABLE VacXin (
    ...
    CONSTRAINT CK_VacXin_NSX CHECK (
        NgaySanXuat <= CAST(GETDATE () AS date)
    ),
...
CREATE TABLE VacXin_ChiNhanh (
    ...
    MaCN varchar(10) NOT NULL,
    TonKho int NOT NULL,
    CONSTRAINT PK_VX_CN PRIMARY KEY (MaVacXin, MaCN),
...
-- 4.6 Phi?u tiêm phòng
CREATE TABLE PhieuDatDVTiemPhong (
    MaPhieuDV varchar(10) NOT NULL,
...
CREATE TABLE DanhSachVacXin (
    ...
    CONSTRAINT FK_DanhSachVX_Phieu FOREIGN KEY (MaPhieuDV) REFERENCES PhieuDatDVTiemPhong (MaPhieuDV),
...
CREATE TABLE HoaDon (
    ...
    CONSTRAINT CK_HoaDon_HTTT CHECK (
        HinhThucThanhToan IN (N'Chuy?n kho?n', N'Ti?n m?t')
    ) -- 2 hình th?c:contentReference[oaicite:17]{index=17}
);
GO

-- 5.2 Chi ti?t hóa don
CREATE TABLE ChiTietHoaDon (
    MaHoaDon varchar(10) NOT NULL,
    MaPhieuDV varchar(10) NOT NULL,
    TongTienTungDV int NOT NULL,
